...
# Youwol clients
from yw_clients.common.headers import YouwolHeaders, generate_headers_downstream
from yw_clients.common.json_utils import AnyDict, to_json
from yw_clients.context.models import (
    ContextReporter,
    DataType,
    HeadersFwdSelector,
    JsonLike,
...
T = TypeVar("T")
"""
Generic type parameter for the :class:`Context <yw_clients.context.context.Context>` class.
...
class Context(Generic[T]):
    ...
    :attr:`env <yw_clients.context.context.Context.env>`, a 'compile-time' resolved contextual information.
    """
    ...
    :meth:`start_ep <yw_clients.context.context.Context.start_ep>`), it stores the original
    ...
    :attr:`headers <yw_clients.context.context.Context.headers>`.
    """
    ...
    async def debug(
        ...
        """
        Log information with severity :attr:`DEBUG <yw_clients.context.models.LogLevel.DEBUG>`.
    ...
    async def info(
        ...
        """
        Log information with severity :attr:`INFO <yw_clients.context.models.LogLevel.INFO>`.
    ...
    async def warning(
        ...
        """
        Log information with severity :attr:`WARNING <yw_clients.context.models.LogLevel.WARNING>`.
    ...
    async def error(
        ...
        """
        Log information with severity :attr:`ERROR <yw_clients.context.models.LogLevel.ERROR>`.
    ...
    async def failed(
        ...
        """
        Log information with severity :attr:`ERROR <yw_clients.context.models.LogLevel.ERROR>` and
    ...
    async def future(
        ...
        """
        Log information with severity :attr:`INFO <yw_clients.context.models.LogLevel.INFO>` and
...
"""
Signature for :class:`scoped context <yw_clients.context.context.ScopedContext>`'s callable when
...
class ContextFactory:
    ...
    @staticmethod
    def proxied_backend_context(
        request: Request, yw_url: str, **kwargs
    ) -> ProxiedBackendContext:
        """
        ...
        *  defines the context's `data_reporter` attribute using a reporter that forward log's entries to the youwol
        ...
        class PyYwReporter(ContextReporter):
            ...
            def __init__(self, channel: Literal["logs", "data"]):
                ...
                self.url = f"{yw_url}/admin/system/{self.channel}"
        ...
        http_clients = ProxiedBackendCtxEnv(
            assets_gateway=AssetsGatewayClient(
                url_base=f"{yw_url}/api/assets-gateway",
                request_executor=AioHttpExecutor(),
            ),
            sessions_storage=CdnSessionsStorageClient(
                url_base=f"{yw_url}/api/cdn-sessions-storage",
                request_executor=AioHttpExecutor(),
            ),
        )
        return Context(
