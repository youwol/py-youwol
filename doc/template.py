...
from youwol.utils import parse_json, AnyDict
...
def format_pythons(project: AnyDict, indent: int) -> str:
    keyword = 'Programming Language :: Python :: '
    versions = [d.split(keyword)[1] for d in project['classifiers'] if keyword in d]
    return format_list([v for v in versions if v != "3"], indent)


def format_list(items: Iterable[str], indent: int) -> str:
    return f"[ {functools.reduce(
        lambda acc, e: acc + f"\n{'\t'*indent}" + f"\"{e}\",", items, "")} \n{'\t'*(indent-1)}],"
...
export const youwolInfo = {{
    ...
    pythons: {format_pythons(data['project'], 2)}
}}
"""
...
shutil.copyfile(
    src=py_yw_folder / 'CHANGELOG.md',
    dst=folder_path / 'assets' / 'CHANGELOG.md'
)
