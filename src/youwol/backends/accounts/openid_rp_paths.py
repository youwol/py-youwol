...
async def authorization_flow(
    ...
    url, code_verifier = await client.auth_flow_url(
        state=state_uuid, redirect_uri=redirect_uri, login_hint=login_hint
    )
    conf.pkce_cache.set(
        state_uuid,
        {"target_uri": target_uri, "code_verifier": code_verifier},
...
async def authorization_flow_callback(
    ...
    session = SessionHandler(
        jwt_cache=conf.jwt_cache, session_uuid=(yw_jwt if yw_jwt else str(uuid.uuid4()))
    )
    ...
    response = RedirectResponse(url=(cached_state["target_uri"]), status_code=307)
    response.set_cookie(
        "yw_jwt",
        session.get_uuid(),
