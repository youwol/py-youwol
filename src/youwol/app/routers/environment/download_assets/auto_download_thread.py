...
class AssetDownloadCompletionReporter(ContextReporter):
    ...
    async def log(self, entry: LogEntry):
        ...
        Parameter:
            ...
            download processing.
        """
        if DownloadEvent.__name__ in entry.labels and (
            entry.data["type"] == self.SUCCEEDED_TAG
            or entry.data["type"] == self.FAILED_TAG
        ):
        ...
        while True:
            ...
            if entry.data["rawId"] == raw_id:
                return FinalDownloadStatus(
                    ...
                    succeeded=entry.data["type"] == self.SUCCEEDED_TAG,
