...
class JwtProviderPyYouwol(JwtProvider):
    ...
    *  'request = context.request' used in get_auth_token_cookie
    ...
    async def get_auth_token(
        ...
        if isinstance(authentication, BrowserAuth):
            return await JwtProviderPyYouwol.get_auth_token_cookie(
                auth_provider=auth_provider, context=context
            )

        raise RuntimeError("Authorization mode not managed")

    @staticmethod
    async def get_auth_token_direct(
        auth_provider: AuthorizationProvider,
        authentication: DirectAuth,
        context: Context,
    ):
        username = authentication.userName
        dependencies = {
            "username": username,
            "openIdClient": f"{auth_provider.openidClient}",
            "type": "auth_token",
        }

        cached_token = next(
            (c for c in JwtProviderPyYouwol.__tokens_cache if c.is_valid(dependencies)),
            None,
        )
        if cached_token:
        ...
        try:
            token = (
                ...
                .direct_flow(username=username, password=authentication.password)
            )
        ...
        JwtProviderPyYouwol.__tokens_cache.append(
            DeadlinedCache(
                value=access_token, deadline=deadline, dependencies=dependencies
            )
        )

        await context.info(
            text="Access token renewed",
            data={
                "openIdClient": auth_provider.openidClient,
    ...
    @staticmethod
    async def get_auth_token_cookie(
        auth_provider: AuthorizationProvider, context: Context
    ):
        ...
        return await JwtProviderCookie(
            ...
            openid_infos=OidcInfos(
                base_uri=auth_provider.openidBaseUrl, client=auth_provider.openidClient
            ),
