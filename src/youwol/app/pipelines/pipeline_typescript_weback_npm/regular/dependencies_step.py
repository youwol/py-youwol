...
from youwol.app.pipelines.pipeline_typescript_weback_npm.regular.models import InputDataDependency
from youwol.app.routers.projects.implementation import get_project_configuration
from youwol.app.routers.projects.models_project import Manifest, PipelineStepStatus, ExplicitNone, \
...
def patch_pipeline_generated_module(dependency: InputDataDependency, module_path: Path, dist_folder: Path):
    package_json = parse_json(module_path / 'package.json')
    if package_json['types'] == 'src/index.ts' and not (dist_folder / 'src').exists():
        copy_tree(source=dependency.project.path / 'src', destination=module_path / 'src', replace=True)
...
async def get_input_data(project: Project, flow_id: str, context: Context):
    ...
    async with context.start(action="get_input_data") as ctx:  # type: Context
        ...
        return {dependency.name: InputDataDependency(
            project=dependency,
...
class DependenciesStep(PipelineStep):
    ...
    async def get_status(self, project: Project, flow_id: str,
        ...
        async with context.start(action="get status of project's dependencies") as ctx:
            ...
            diff = DeepDiff(all_dependencies, last_manifest.cmdOutputs.get('allDependencies', {}))
            if diff:
                await ctx.info(text="dependencies have changed in package.json", data=diff)
            ...
            if not to_sync:
                await ctx.info(text='no dependencies to synchronized', data=to_json(last_manifest))
            ...
            synced_dist_artifacts = [k for k, v in data.items()
                                     if k in prev_checksums and prev_checksums[k] == v.checksum]
            not_synced_dist_artifacts = [k for k, v in data.items() if k not in synced_dist_artifacts]

            await ctx.info(text='synchronization of dist artifacts',
                           data={'synced': synced_dist_artifacts, 'notSynced': not_synced_dist_artifacts},
                           )
    ...
    async def execute_run(self, project: Project, flow_id: FlowId, context: Context):
        ...
        async with context.start(action="run synchronization of workspace dependencies") as ctx:
            ...
            data = await get_input_data(project=project, flow_id=flow_id, context=ctx)
            existing_modules_to_maybe_sync = [name for name in data.keys()
            ...
            for name, p in data.items():
                if name not in to_sync:
                    ...
                    continue
                if name in to_sync:
                    await ctx.info(text=f"sync package {name}",
                                   data={"source": p.dist_folder, "destination": destination_folders[name]})
                copy_tree(source=p.dist_folder, destination=destination_folders[name], replace=True)
                patch_pipeline_generated_module(dependency=p, module_path=destination_folders[name],
                                                dist_folder=p.dist_folder)

            selected_data = {k: v for k, v in data.items() if k in to_sync}
            all_files = functools.reduce(lambda acc, e: acc + e.dist_files, selected_data.values(), [])
            ...
            return {
                ...
                'actualChecksums': {
                    name: DependenciesStep.node_module_checksum(project=project, name=name)
