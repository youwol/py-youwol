    ...
    (0,rxjs_operators__WEBPACK_IMPORTED_MODULE_0__.filter)(({ fromLayerId, toLayerId }) => fromLayerId != undefined && toLayerId != undefined), (0,rxjs_operators__WEBPACK_IMPORTED_MODULE_0__.map)(({ fromLayerId, toLayerId }) => ({ fromLayer: appStore.getLayer(fromLayerId), toLayer: appStore.getLayer(toLayerId) })), (0,rxjs_operators__WEBPACK_IMPORTED_MODULE_0__.map)(({ fromLayer, toLayer }) => {
        ...
        if (toLayer.getAllChildren(appStore.project.workflow).includes(fromLayer)) {
            let targetLayer = toLayer.getDirectChildren(appStore.project.workflow)
                .find(layer => layer instanceof _youwol_flux_core__WEBPACK_IMPORTED_MODULE_1__.GroupModules.Module && (layer == fromLayer || layer.getModuleIds().includes(fromLayer.moduleId)));
...
function drawExpandedGroup(layerId, drawingArea, appStore) {
    ...
    let groupMdle = appStore.project.workflow.modules
        .find(m => m instanceof _youwol_flux_core__WEBPACK_IMPORTED_MODULE_2__.GroupModules.Module && m.moduleId == activateLayer.moduleId);
...
function addConnection(connection, project, allSubscriptions) {
    ...
    let workflow = new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.Workflow({
        modules,
        connections,
        plugins: project.workflow.plugins
    });
    workflow = new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.Workflow({
        modules,
        connections,
        plugins: project.workflow.plugins
    });
    let projectNew = new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.Project({
        ...project,
        ...{ workflow: workflow }
    });
    return projectNew;
...
function updateRequirements(loadingGraph, project) {
    ...
    let newProject = new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.Project({
        ...project,
        ...{ requirements }
    });
    return newProject;
...
function addDescriptionBox(descriptionBox, project) {
    ...
    let projectNew = new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.Project({
        ...project,
        ...{
            builderRendering: new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.BuilderRendering(project.builderRendering.modulesView, project.builderRendering.connectionsView, boxes)
        }
    });
    return projectNew;
}
...
function deleteDescriptionBox(descriptionBox, project) {
    ...
    let projectNew = new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.Project({
        ...project,
        ...{
            builderRendering: new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.BuilderRendering(project.builderRendering.modulesView, project.builderRendering.connectionsView, toKeeps)
        }
    });
    return projectNew;
}
...
/* harmony export */   "applyHtmlLayout": () => (/* binding */ applyHtmlLayout),
/* harmony export */   "applyHtmlCss": () => (/* binding */ applyHtmlCss),
...
/* harmony import */ var _app_store_modules_group__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./app-store-modules-group */ "./builder-editor/builder-state/app-store-modules-group.ts");




function getLayer(workflow, parentGroup, group, id) {
    if (group.moduleId === id)
        return [group, parentGroup];
    return group.getDirectChildren(workflow)
        .filter(mdle => mdle instanceof _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.GroupModules.Module)
        .map((mdle) => {
...
function applyHtmlLayout(workflow, html) {
    ...
    let newComponents = componentsToEventuallyUpdate.map((component) => {
        let deepHtml = componentsDiv[component.moduleId];
        if (!deepHtml)
            return undefined;
        component.getDirectChildren(workflow)
            .filter(child => child instanceof _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.Component.Module)
        ...
        if (html == component.getPersistentData().html)
            return undefined;
        let newComponent = (0,_app_store_modules_group__WEBPACK_IMPORTED_MODULE_3__.updateComponent)(component, { html });
        return newComponent;
    })
        .filter(d => d);
    debugSingleton.debugOn &&
    ...
    let newWorkflow = new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.Workflow({
        ...workflow,
...
function applyHtmlCss(rootComponent, workflow, css) {
    let debugSingleton = _app_debug_environment__WEBPACK_IMPORTED_MODULE_2__.AppDebugEnvironment.getInstance();
    let wrapDiv = (html) => {
    ...
    let isEquivalent = (oldRules, newRules) => {
    ...
    let newComponents = workflow.modules
        .filter(mdle => mdle instanceof _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.Component.Module)
        ...
        for (let i = 0; i < styleSheet.rules.length; i++) {
            let cssRule = styleSheet.rules[i];
            let elements = Array
                ...
                let childrenComponentId = component
                    ...
                    .filter(c => c instanceof _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.Component.Module)
        ...
        let oldCssRules = component.getPersistentData().css.split("\n");
        if (isEquivalent(oldCssRules, [...cssRules]))
            return undefined;
        let css = [...cssRules].reduce((acc, e) => acc + "\n" + e, "");
        let newComponent = (0,_app_store_modules_group__WEBPACK_IMPORTED_MODULE_3__.updateComponent)(component, { css });
        return newComponent;
    })
        .filter(d => d);
...
function cloneLayerTree(layerTree, filter = (mdleId) => true, layout = (layout) => layout.html, css = (layout) => layout.css) {
    return new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.LayerTree({
        ...layerTree,
        ...{
...
function createLayer(title, modules, project, currentLayerId, Factory, configuration, workflowGetter, environment) {
    ...
    let newGrpMdle = new Factory.Module({
        moduleId: Factory.id.replace("@", "_") + "_" + (0,_utils__WEBPACK_IMPORTED_MODULE_1__.uuidv4)(),
        configuration: new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.ModuleConfiguration({
            title,
            description: '',
    ...
    let parentGroup = project.workflow.modules.find(mdle => mdle.moduleId == currentLayerId);
    let parentGroupUpdated = (0,_app_store_modules_group__WEBPACK_IMPORTED_MODULE_3__.updateGroup)(parentGroup, {
    ...
    let moduleView = new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.ModuleView(newGrpMdle.moduleId, xWorld, yWorld, Factory);
    let moduleViews = [...project.builderRendering.modulesView, moduleView];
    let projectNew = new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.Project({
        ...project,
        ...{
            workflow,
            builderRendering: new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.BuilderRendering(moduleViews, project.builderRendering.connectionsView, project.builderRendering.descriptionsBoxes),
        }
    });
...
function updateGroup(original, persistentFields) {
    let persistentData = new original.Factory.PersistentData({
        ...original.getPersistentData(),
        ...persistentFields
    });
    let grpConfiguration = new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.ModuleConfiguration({ ...original.configuration, data: persistentData });
...
function updateComponent(original, persistentFields) {
    return updateGroup(original, persistentFields);
}
function getDisplayedModulesView(group, parentGroup, appStore, project) {
...
function addModule(Factory, coors, project, activeLayerId, ready$, environment) {
    ...
    let persistentData = new groupMdle.Factory.PersistentData({
        ...
        moduleIds: groupMdle.getPersistentData().getModuleIds().concat(moduleId)
    });
    let grpConfiguration = new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_2__.ModuleConfiguration({ ...groupMdle.configuration, data: persistentData });
    ...
    let workflow = new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_2__.Workflow({
        ...project.workflow,
        ...{
            modules: project.workflow.modules
    ...
    let projectNew = new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_2__.Project({
        ...project,
        ...{
            workflow,
            builderRendering,
            runnerRendering: new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_2__.RunnerRendering(layout, project.runnerRendering.style)
        }
    });
    debugSingleton.debugOn &&
        debugSingleton.logWorkflowBuilder({
...
function alignH(moduleIds, project, ready$) {
    ...
    let projectNew = new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_2__.Project({
        ...project,
        ...{
            builderRendering: new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_2__.BuilderRendering(modulesViewToKeep.concat(newViews), project.builderRendering.connectionsView, project.builderRendering.descriptionsBoxes)
        }
    });
    return projectNew;
}
...
function duplicateModules(modules, project, ready$) {
    ...
    let parentGroups = newModules
        ...
        let parent = project.workflow.modules
            .find(mdle => mdle instanceof _youwol_flux_core__WEBPACK_IMPORTED_MODULE_2__.GroupModules.Module &&
            mdle.getPersistentData().getModuleIds().includes(moduleDuplicated.moduleId));
    ...
    let newGroups = Object.entries(parentGroups).map(([parentId, moduleIds]) => {
        let parent = project.workflow.modules.find(mdle => mdle.moduleId == parentId);
        let persistentData = parent.getPersistentData();
        let newGroup = (0,_app_store_modules_group__WEBPACK_IMPORTED_MODULE_3__.updateGroup)(parent, { moduleIds: persistentData.getModuleIds().concat(...moduleIds) });
        return newGroup;
    });
    ...
    let projectNew = new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_2__.Project({
        ...project,
        ...{
            workflow: new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_2__.Workflow({
                modules: wf.modules
...
function moveModules(modulesPosition, moduleViews, project, implicitModules) {
    ...
    let projectNew = new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_2__.Project({
        ...project,
        ...{
            builderRendering: new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_2__.BuilderRendering(modulesViewToKeep.concat(modulesViewNew), project.builderRendering.connectionsView, descriptionBoxes)
        }
    });
    return projectNew;
}
function deleteModules(modulesDeleted, project) {
    ...
    let grpMdlesDeleted = modulesDeleted.filter(mdle => mdle instanceof _youwol_flux_core__WEBPACK_IMPORTED_MODULE_2__.GroupModules.Module);
    let inGroupModulesDeleted = grpMdlesDeleted
        .map((grpMdle) => grpMdle.getAllChildren(project.workflow))
        .flat();
    ...
    let newGroups = project.workflow.modules
        .filter(mdle => mdle instanceof _youwol_flux_core__WEBPACK_IMPORTED_MODULE_2__.GroupModules.Module)
        .filter((mdle) => !grpMdlesDeleted.includes(mdle))
        .map((mdle) => {
        ...
        if (childIds.length == filtered.length)
            return undefined;
        return (0,_app_store_modules_group__WEBPACK_IMPORTED_MODULE_3__.updateGroup)(mdle, { moduleIds: filtered });
    })
        .filter(d => d);
    let modules = project.workflow.modules
    ...
    let projectNew = new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_2__.Project({
        ...
        ...{
            workflow,
            builderRendering: new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_2__.BuilderRendering(modulesView, project.builderRendering.connectionsView, descriptionBoxes)
        }
    });
    return projectNew;
}
...
/* harmony import */ var _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(_youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var _app_debug_environment__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./app-debug.environment */ "./builder-editor/builder-state/app-debug.environment.ts");
/* harmony import */ var _app_store_layer__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./app-store-layer */ "./builder-editor/builder-state/app-store-layer.ts");
...
function setRenderingLayout(layout, project) {
    ...
    let workflow = (0,_app_store_layer__WEBPACK_IMPORTED_MODULE_2__.applyHtmlLayout)(project.workflow, layout);
    let projectNew = new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.Project({
        ...project,
        ...{
            workflow: workflow,
            runnerRendering: new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.RunnerRendering(layout, project.runnerRendering.style)
        }
    });
    return projectNew;
}
function setRenderingStyle(rootComponent, style, project) {
    let debugSingleton = _app_debug_environment__WEBPACK_IMPORTED_MODULE_1__.AppDebugEnvironment.getInstance();
    debugSingleton.debugOn &&
        debugSingleton.logWorkflowBuilder({
            level: _app_debug_environment__WEBPACK_IMPORTED_MODULE_1__.LogLevel.Info,
    ...
    let workflow = (0,_app_store_layer__WEBPACK_IMPORTED_MODULE_2__.applyHtmlCss)(rootComponent, project.workflow, style);
    ...
    let projectNew = new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.Project({
        ...
        ...{
            workflow,
            runnerRendering: new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.RunnerRendering(project.runnerRendering.layout, style)
        }
    });
    return projectNew;
}
...
/* harmony import */ var rxjs__WEBPACK_IMPORTED_MODULE_19__ = __webpack_require__(/*! rxjs */ "rxjs");
/* harmony import */ var rxjs__WEBPACK_IMPORTED_MODULE_19___default = /*#__PURE__*/__webpack_require__.n(rxjs__WEBPACK_IMPORTED_MODULE_19__);
...
class AppStore {
    constructor(environment, appObservables, appBuildViewObservables) {
        ...
        this.project = new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.Project({
            name: "new project",
            schemaVersion: "1.0",
            description: "",
            requirements: new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.Requirements([], [], {}, {}),
            workflow: new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.Workflow({
                modules: [],
                connections: [],
                plugins: []
            }),
            builderRendering: new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.BuilderRendering([], [], []),
            runnerRendering: new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.RunnerRendering("", "")
        });
        this.workflow$ = new rxjs__WEBPACK_IMPORTED_MODULE_19__.Subject();
        this.rootLayerId = 'Component_root-component';
        ...
        _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.GroupModules.BuilderView.notifier$.pipe((0,rxjs_operators__WEBPACK_IMPORTED_MODULE_13__.filter)((event) => event.type == "closeLayer")).subscribe(d => this.selectActiveLayer(this.getParentLayer(d.data).moduleId));
        _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.Component.BuilderView.notifier$.pipe((0,rxjs_operators__WEBPACK_IMPORTED_MODULE_13__.filter)((event) => event.type == "closeLayer")).subscribe(d => this.selectActiveLayer(this.getParentLayer(d.data).moduleId));
    ...
    loadProject(projectId, project, onEvent) {
        ...
        let project$ = (0,_youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.loadProjectDependencies$)(project, this.environment, onEvent).pipe((0,rxjs_operators__WEBPACK_IMPORTED_MODULE_13__.map)(({ project, packages }) => {
            return (0,_youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.createProject)(project, packages, this.workflow$, this.allSubscriptions, this.environment);
        }));
        this.initializeProject(project$);
    }
    loadProjectId(projectId) {
        this.projectId = projectId;
        let project$ = (0,_youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.loadProjectDatabase$)(projectId, this.workflow$, this.allSubscriptions, this.environment);
        this.initializeProject(project$);
    }
    loadProjectURI(projectURI) {
        this.projectId = undefined;
        let project$ = (0,_youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.loadProjectURI$)(projectURI, this.workflow$, this.allSubscriptions, this.environment);
        this.initializeProject(project$);
    }
    initializeProject(project$) {
        project$.subscribe(({ project, packages }) => {
            this.appObservables.packagesUpdated$.next(packages);
            let rootComponent = project.workflow.modules.find(mdle => mdle.moduleId == this.rootLayerId);
    ...
    updateModule(mdle, configuration, unselect = true) {
    ...
    selectConnection(connection) {
        this.unselect();
        this.connectionSelected = connection instanceof _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.Connection
            ? connection
    ...
    deleteModules(modulesDeleted) {
        ...
        let rootComponent = project.workflow.modules
            .find(mdle => mdle.moduleId == this.rootLayerId);
        if (!(0,_app_store_layer__WEBPACK_IMPORTED_MODULE_11__.getLayer)(project.workflow, rootComponent, rootComponent, this.activeLayerId)) {
    ...
    getActiveLayer() {
        return (0,_app_store_layer__WEBPACK_IMPORTED_MODULE_11__.getLayer)(this.project.workflow, undefined, this.getModule(this.rootLayerId), this.activeLayerId)[0];
    }
    getRootLayer() {
        return (0,_app_store_layer__WEBPACK_IMPORTED_MODULE_11__.getLayer)(this.project.workflow, undefined, this.getModule(this.rootLayerId), this.rootLayerId)[0];
    }
    getLayer(layerId) {
        let a = (0,_app_store_layer__WEBPACK_IMPORTED_MODULE_11__.getLayer)(this.project.workflow, undefined, this.getModule(this.rootLayerId), layerId);
        if (a == undefined) {
    ...
    getParentLayer(layerId) {
        return (0,_app_store_layer__WEBPACK_IMPORTED_MODULE_11__.getLayer)(this.project.workflow, undefined, this.getModule(this.rootLayerId), layerId)[1];
    }
    ...
    getDisplayedModulesView() {
        let [activeGroup, parentGroup] = (0,_app_store_layer__WEBPACK_IMPORTED_MODULE_11__.getLayer)(this.project.workflow, undefined, this.getModule(this.rootLayerId), this.activeLayerId);
        return (0,_app_store_modules_group__WEBPACK_IMPORTED_MODULE_12__.getDisplayedModulesView)(activeGroup, parentGroup, this, this.project);
    }
    getGroupModule(layerId) {
        return this.project.workflow.modules
            .find(m => m instanceof _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.GroupModules.Module && m.moduleId == layerId);
    }
    getParentGroupModule(moduleId) {
        ...
        return this.project.workflow.modules
            .filter(mdle => mdle instanceof _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.GroupModules.Module)
            .find((grp) => grp.getModuleIds().includes(mdle.moduleId));
    }
    ...
    setConnectionView(connection, properties, unselect = true) {
    ...
    setRenderingStyle(style, asNewState = true) {
        let rootComponent = this.getLayer(this.rootLayerId);
        let project = (0,_app_store_runner_rendering__WEBPACK_IMPORTED_MODULE_10__.setRenderingStyle)(rootComponent, style, this.project);
...
/* harmony export */   "applyHtmlCss": () => (/* reexport safe */ _app_store_layer__WEBPACK_IMPORTED_MODULE_7__.applyHtmlCss),
/* harmony export */   "applyHtmlLayout": () => (/* reexport safe */ _app_store_layer__WEBPACK_IMPORTED_MODULE_7__.applyHtmlLayout),
...
/* harmony export */   "updateComponent": () => (/* reexport safe */ _app_store_modules_group__WEBPACK_IMPORTED_MODULE_9__.updateComponent),
/* harmony export */   "updateGroup": () => (/* reexport safe */ _app_store_modules_group__WEBPACK_IMPORTED_MODULE_9__.updateGroup),
...
function packageAssetProject(project) {
    let publishRequest = {
        ...
        project: (0,_factory_utils__WEBPACK_IMPORTED_MODULE_1__.toProjectData)(project)
    };
...
function plugBuilderViewsSignals(modules, actions, redirections$) {
    modules.forEach(mdle => {
        ...
        if (mdle.Factory.BuilderView.notifier$ && !mdle.Factory.consumersData.FluxAppstore.notifiersPluged) {
            mdle.Factory.BuilderView.notifier$.pipe((0,rxjs_operators__WEBPACK_IMPORTED_MODULE_0__.filter)((event) => event.type == "configurationUpdated"))
                .subscribe((event) => {
...
let ALL_ACTIONS = {
    newModules: {
        createNode: (state) => new _nodes__WEBPACK_IMPORTED_MODULE_4__.NewModulesNode(),
    ...
    journals: {
        createNode: (state) => new _nodes__WEBPACK_IMPORTED_MODULE_4__.JournalsNode(),
        applicable: (state) => {
    ...
    documentation: {
        createNode: (state) => new _nodes__WEBPACK_IMPORTED_MODULE_4__.DocumentationNode(state),
        applicable: (state) => {
...
/* harmony export */   "DocumentationNode": () => (/* reexport safe */ _nodes__WEBPACK_IMPORTED_MODULE_1__.DocumentationNode),
...
/* harmony export */   "ResourceNode": () => (/* reexport safe */ _nodes__WEBPACK_IMPORTED_MODULE_1__.ResourceNode)
/* harmony export */ });
...
/* harmony export */   "DocumentationNode": () => (/* binding */ DocumentationNode),
/* harmony export */   "ResourceNode": () => (/* binding */ ResourceNode)
/* harmony export */ });
/* harmony import */ var _youwol_fv_tree__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! @youwol/fv-tree */ "@youwol/fv-tree");
/* harmony import */ var _youwol_fv_tree__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(_youwol_fv_tree__WEBPACK_IMPORTED_MODULE_0__);
...
class DocumentationNode extends ContextTreeNode {
    static createChildren(appStore) {
...
class ResourceNode extends ContextTreeNode {
...
/* harmony export */   "DocumentationNode": () => (/* reexport safe */ _context_menu_index__WEBPACK_IMPORTED_MODULE_0__.DocumentationNode),
...
/* harmony export */   "ResourceNode": () => (/* reexport safe */ _context_menu_index__WEBPACK_IMPORTED_MODULE_0__.ResourceNode),
...
/* harmony import */ var _youwol_flux_core__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! @youwol/flux-core */ "@youwol/flux-core");
/* harmony import */ var _youwol_flux_core__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(_youwol_flux_core__WEBPACK_IMPORTED_MODULE_1__);
/* harmony import */ var _youwol_flux_view__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! @youwol/flux-view */ "@youwol/flux-view");
/* harmony import */ var _youwol_flux_view__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(_youwol_flux_view__WEBPACK_IMPORTED_MODULE_2__);
/* harmony import */ var _views_module_settings_view__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./views/module-settings.view */ "./builder-editor/views/module-settings.view.ts");
/* harmony import */ var _views_connection_settings_view__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./views/connection-settings.view */ "./builder-editor/views/connection-settings.view.ts");





function moduleControls(mdle, appStore) {
    let state = new _views_module_settings_view__WEBPACK_IMPORTED_MODULE_3__.ModuleSettingsState(mdle, appStore);
    return new _views_module_settings_view__WEBPACK_IMPORTED_MODULE_3__.ModuleSettingsView(state);
}
function connectionControls(connection, appStore) {
    let state = new _views_connection_settings_view__WEBPACK_IMPORTED_MODULE_4__.ConnectionSettingsState(connection, appStore);
    return new _views_connection_settings_view__WEBPACK_IMPORTED_MODULE_4__.ConnectionSettingsView(state);
}
function createAttributesPanel(appStore, appObservables) {
    (0,rxjs__WEBPACK_IMPORTED_MODULE_0__.merge)(appObservables.moduleSelected$, appObservables.connectionSelected$).subscribe((d) => {
        let virtualDOM = {
            ...
            children: [
                d instanceof _youwol_flux_core__WEBPACK_IMPORTED_MODULE_1__.ModuleFlux
                    ? moduleControls(d, appStore)
                    : connectionControls(d, appStore)
            ]
        };
        document.getElementById("panels-container-builder").innerHTML = "";
        let panelDiv = document.getElementById("panels-container-builder");
        /*let virtualDOM = d.moduleId ?
            moduleControls(d as ModuleFlux, appStore) :
            connectionControls(d as Connection, appStore)*/
        panelDiv.appendChild((0,_youwol_flux_view__WEBPACK_IMPORTED_MODULE_2__.render)(virtualDOM));
    });
...
/***/ "./builder-editor/views/auto-form.view.ts":
/*!************************************************!*\
  !*** ./builder-editor/views/auto-form.view.ts ***!
  \************************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "AutoForm": () => (/* binding */ AutoForm)
/* harmony export */ });
/* harmony import */ var rxjs__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! rxjs */ "rxjs");
/* harmony import */ var rxjs__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(rxjs__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var rxjs_operators__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! rxjs/operators */ "rxjs/operators");
/* harmony import */ var rxjs_operators__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(rxjs_operators__WEBPACK_IMPORTED_MODULE_1__);
/* harmony import */ var _youwol_fv_button__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! @youwol/fv-button */ "@youwol/fv-button");
/* harmony import */ var _youwol_fv_button__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(_youwol_fv_button__WEBPACK_IMPORTED_MODULE_2__);
/* harmony import */ var _youwol_fv_input__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! @youwol/fv-input */ "@youwol/fv-input");
/* harmony import */ var _youwol_fv_input__WEBPACK_IMPORTED_MODULE_3___default = /*#__PURE__*/__webpack_require__.n(_youwol_fv_input__WEBPACK_IMPORTED_MODULE_3__);
/* harmony import */ var _youwol_fv_tabs__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! @youwol/fv-tabs */ "@youwol/fv-tabs");
/* harmony import */ var _youwol_fv_tabs__WEBPACK_IMPORTED_MODULE_4___default = /*#__PURE__*/__webpack_require__.n(_youwol_fv_tabs__WEBPACK_IMPORTED_MODULE_4__);
/* harmony import */ var _youwol_flux_view__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! @youwol/flux-view */ "@youwol/flux-view");
/* harmony import */ var _youwol_flux_view__WEBPACK_IMPORTED_MODULE_5___default = /*#__PURE__*/__webpack_require__.n(_youwol_flux_view__WEBPACK_IMPORTED_MODULE_5__);
...
(function (AutoForm) {
    class State {
        constructor(inputValues$, schema, elementsViewFactory = AutoForm.viewFactory) {
            this.schema = schema;
            this.elementsViewFactory = elementsViewFactory;
            this.outputValues$ = new Array();
            this.inputValues$ = (inputValues$ instanceof rxjs__WEBPACK_IMPORTED_MODULE_0__.BehaviorSubject)
                ? inputValues$
                : new rxjs__WEBPACK_IMPORTED_MODULE_0__.BehaviorSubject(inputValues$);
            console.log("Scemas", schema);
            this.currentValue$ = new rxjs__WEBPACK_IMPORTED_MODULE_0__.BehaviorSubject(this.inputValues$.getValue());
        }
    }
    AutoForm.State = State;
    AutoForm.viewFactory = [
        {
            test: (value) => value.type == "Boolean",
            view: (value$) => {
                _youwol_fv_button__WEBPACK_IMPORTED_MODULE_2__.Switch.View.defaultRadius = 10;
                return new _youwol_fv_button__WEBPACK_IMPORTED_MODULE_2__.Switch.View({
                    state: new _youwol_fv_button__WEBPACK_IMPORTED_MODULE_2__.Switch.State(value$)
                });
            }
        },
        {
            test: (value) => value.metadata.type == "color",
            view: (value$, description) => {
                return new _youwol_fv_input__WEBPACK_IMPORTED_MODULE_3__.ColorPicker.View({
                    state: new _youwol_fv_input__WEBPACK_IMPORTED_MODULE_3__.ColorPicker.State(value$)
                });
            }
        },
        {
            test: (value) => value.metadata.enum,
            view: (value$, description) => {
                let items = description.metadata.enum.map(text => new _youwol_fv_input__WEBPACK_IMPORTED_MODULE_3__.Select.ItemData(text, text));
                return new _youwol_fv_input__WEBPACK_IMPORTED_MODULE_3__.Select.View({
                    state: new _youwol_fv_input__WEBPACK_IMPORTED_MODULE_3__.Select.State(items, value$),
                    class: 'w-100 ',
        ...
        },
        {
            ...
            view: (value$, description) => {
                let hovered$ = new rxjs__WEBPACK_IMPORTED_MODULE_0__.BehaviorSubject(false);
                let state = new _youwol_fv_input__WEBPACK_IMPORTED_MODULE_3__.Slider.State({ min: description.metadata.min, max: description.metadata.max, value: value$, count: 1000 });
                return {
                    class: 'd-flex',
                    children: [
                        {
                            tag: 'input',
                            class: (0,_youwol_flux_view__WEBPACK_IMPORTED_MODULE_5__.attr$)(hovered$, (isHovered) => isHovered ? 'w-100' : 'w-25'),
                            type: 'number',
                            value: (0,_youwol_flux_view__WEBPACK_IMPORTED_MODULE_5__.attr$)(value$, (v) => v),
                            onchange: (event) => value$.next(Number(event.target.value)),
                            onmouseenter: () => hovered$.next(true),
                            onmouseleave: () => hovered$.next(false),
                        },
                        new _youwol_fv_input__WEBPACK_IMPORTED_MODULE_3__.Slider.View({ state, class: "w-75" })
                    ]
                };
            }
        },
        {
            test: (value) => value.type == "Number",
            view: (value$) => {
                return {
                    tag: 'input',
                    type: 'number',
    ...
    class View {
        constructor({ state, ...rest }) {
            ...
            this.state = state;
            this.children = [
                (0,_youwol_flux_view__WEBPACK_IMPORTED_MODULE_5__.child$)(state.inputValues$, (formDescription) => {
                    return groupItems(Object.entries(state.schema), formDescription, [], state);
                }, {
                    sideEffects: (_, elem) => {
                        let sub = (0,rxjs__WEBPACK_IMPORTED_MODULE_0__.combineLatest)(this.state.outputValues$).subscribe(entries => {
                            let base = (0,lodash__WEBPACK_IMPORTED_MODULE_6__.cloneDeep)(state.inputValues$.getValue());
    ...
    AutoForm.View = View;
    function createAttributesGrid(items, basePath, configurationBase, state
    ...
    function leafView(valueMeta, path, configuration, state
    //observables: Array<Observable<{path:Array<string>,value:ValueType}>>
    ) {
        ...
        let value$ = new rxjs__WEBPACK_IMPORTED_MODULE_0__.BehaviorSubject(value0);
        let view = factory.view(value$, valueMeta);
        let obs = value$.pipe((0,rxjs_operators__WEBPACK_IMPORTED_MODULE_1__.map)(value => ({ path, value })));
        state.outputValues$.push(obs);
        return view;
    }
    function groupItems(items, configurationBase, 
    //observables: Array<Observable<{path:Array<string>,value:ValueType}>>,
    basePath = [], state) {
        let currentItems = items.filter(([k, v]) => !k.includes(".") && isLeaf(v));
        let children = items
            ...
            .reduce((acc, [prefix, val, basePath]) => {
            if (!acc[prefix])
        ...
        let r = Object.entries(children)
            .map(([prefix, { values, basePath }]) => [prefix, groupItems(values, configurationBase, basePath, state)])
        ...
        let tabsData = Object.entries(r).map(([key, val]) => {
            let tabData = new _youwol_fv_tabs__WEBPACK_IMPORTED_MODULE_4__.Tabs.TabData(key, key);
            tabData['view'] = val;
            return tabData;
        });
        ...
        let tabState = new _youwol_fv_tabs__WEBPACK_IMPORTED_MODULE_4__.Tabs.State(tabsData);
        let tabView = new _youwol_fv_tabs__WEBPACK_IMPORTED_MODULE_4__.Tabs.View({
            state: tabState,
...
(function (CodeEditorView) {
    ...
    function fetchCodeMirror$(mode) {
        ...
        return (0,rxjs__WEBPACK_IMPORTED_MODULE_1__.from)(cdn.fetchBundles({ codemirror: { version: '5.52.0' } }, window)).pipe((0,rxjs_operators__WEBPACK_IMPORTED_MODULE_2__.mergeMap)(() => {
            ...
            let promise = cdn.fetchJavascriptAddOn(urls, window);
...
/***/ "./builder-editor/views/connection-settings.view.ts":
/*!**********************************************************!*\
  !*** ./builder-editor/views/connection-settings.view.ts ***!
  \**********************************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
/* harmony export */   "ConnectionSettingsState": () => (/* binding */ ConnectionSettingsState),
/* harmony export */   "ConnectionSettingsView": () => (/* binding */ ConnectionSettingsView)
/* harmony export */ });
...
/* harmony import */ var rxjs_operators__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! rxjs/operators */ "rxjs/operators");
/* harmony import */ var rxjs_operators__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__);
/* harmony import */ var _adaptor_editor_view__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./adaptor-editor.view */ "./builder-editor/views/adaptor-editor.view.ts");
/* harmony import */ var _auto_form_view__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./auto-form.view */ "./builder-editor/views/auto-form.view.ts");
...
ConnectionView = (0,tslib__WEBPACK_IMPORTED_MODULE_5__.__decorate)([
...
let elementViewsFactory = (connection, appStore) => [
    {
        test: (value) => {
        ...
        view: (value$) => {
            return {
                ...
                onclick: () => {
                    _adaptor_editor_view__WEBPACK_IMPORTED_MODULE_3__.AdaptorEditoView.popupModal({
                        connection,
    ...
    ..._auto_form_view__WEBPACK_IMPORTED_MODULE_4__.AutoForm.viewFactory
];
class ConnectionSettingsState {
    constructor(connection, appStore) {
        this.appStore = appStore;
        ...
        let schemaWithValue = (0,_youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.flattenSchemaWithValue)(data);
        Object.keys(schemaWithValue).forEach(k => schemaWithValue[k] = schemaWithValue[k][0]);
        let configurationIn$ = new rxjs__WEBPACK_IMPORTED_MODULE_1__.BehaviorSubject(data);
        this.autoFormState = new _auto_form_view__WEBPACK_IMPORTED_MODULE_4__.AutoForm.State(configurationIn$, schemaWithValue, elementViewsFactory(connection, appStore));
        this.initialSettings$ = new rxjs__WEBPACK_IMPORTED_MODULE_1__.BehaviorSubject(data);
        let sub1 = this.autoFormState.currentValue$.pipe((0,rxjs_operators__WEBPACK_IMPORTED_MODULE_2__.filter)((value) => value['wireless'] != this.initialSettings$.getValue()['wireless']))
            .subscribe((value) => {
        ...
        let sub2 = this.autoFormState.currentValue$.pipe((0,rxjs_operators__WEBPACK_IMPORTED_MODULE_2__.filter)((value) => value['adaptor'] != this.initialSettings$.getValue()['adaptor']))
            .subscribe((value) => {
            this.initialSettings$.next(value);
            let adaptor = new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.Adaptor((0,_youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.uuidv4)(), value.adaptor);
...
class ConnectionSettingsView {
    constructor(state) {
        this.state = state;
        this.class = "h-100 d-flex flex-column";
        this.style = { fontSize: 'smaller' };
        this.children = [
            new _auto_form_view__WEBPACK_IMPORTED_MODULE_4__.AutoForm.View({ state: state.autoFormState, class: 'flex-grow-1 overflow-auto my-1', style: { 'min-height': '0px' } })
        ];
...
/***/ "./builder-editor/views/module-settings.view.ts":
/*!******************************************************!*\
  !*** ./builder-editor/views/module-settings.view.ts ***!
  \******************************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "ModuleSettingsState": () => (/* binding */ ModuleSettingsState),
/* harmony export */   "ModuleSettingsView": () => (/* binding */ ModuleSettingsView)
/* harmony export */ });
/* harmony import */ var _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! @youwol/flux-core */ "@youwol/flux-core");
/* harmony import */ var _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(_youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var _youwol_flux_view__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! @youwol/flux-view */ "@youwol/flux-view");
...
/* harmony import */ var rxjs_operators__WEBPACK_IMPORTED_MODULE_3___default = /*#__PURE__*/__webpack_require__.n(rxjs_operators__WEBPACK_IMPORTED_MODULE_3__);
/* harmony import */ var _auto_form_view__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./auto-form.view */ "./builder-editor/views/auto-form.view.ts");
/* harmony import */ var _code_property_editor_view__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./code-property-editor.view */ "./builder-editor/views/code-property-editor.view.ts");
...
function codeEditorView(faClass, mdle, value$, description) {
    let editorConfiguration = description.metadata.editorConfiguration || {};
    return {
        class: `${faClass} fv-pointer fa-2x fv-hover-bg-background`,
        onclick: () => _code_property_editor_view__WEBPACK_IMPORTED_MODULE_5__.CodePropertyEditorView.popupModal({
            mdle: mdle,
            initialCode: value$.getValue(),
            editorConfiguration,
            onUpdate: (content) => value$.next(content)
...
class ModuleSettingsState {
    constructor(mdle, appStore) {
        this.mdle = mdle;
        this.appStore = appStore;
        this.onTheFlyUpdates$ = new rxjs__WEBPACK_IMPORTED_MODULE_2__.BehaviorSubject(true);
        let schemaWithValue = (0,_youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.flattenSchemaWithValue)(mdle.configuration.data);
        Object.keys(schemaWithValue).forEach(k => schemaWithValue[k] = schemaWithValue[k][0]);
        let configurationIn$ = new rxjs__WEBPACK_IMPORTED_MODULE_2__.BehaviorSubject(mdle.configuration.data);
        this.initialSettings$ = new rxjs__WEBPACK_IMPORTED_MODULE_2__.BehaviorSubject(mdle.configuration.data);
        this.autoFormState = new _auto_form_view__WEBPACK_IMPORTED_MODULE_4__.AutoForm.State(configurationIn$, schemaWithValue, elementViewsFactory(mdle));
        let sub = this.autoFormState.currentValue$.pipe((0,rxjs_operators__WEBPACK_IMPORTED_MODULE_3__.withLatestFrom)(this.onTheFlyUpdates$), (0,rxjs_operators__WEBPACK_IMPORTED_MODULE_3__.filter)(([_, onTheFly]) => onTheFly), (0,rxjs_operators__WEBPACK_IMPORTED_MODULE_3__.filter)(([value]) => JSON.stringify(value) != JSON.stringify(this.initialSettings$.getValue())))
    ...
    applySettings(title = undefined) {
        ...
        let conf = new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.ModuleConfiguration({
...
class ModuleSettingsView {
    ...
    header() {
        return {
            ...
            children: [
                (0,_youwol_flux_view__WEBPACK_IMPORTED_MODULE_1__.child$)(this.state.onTheFlyUpdates$, (onTheFly) => this.updatePolicyView(onTheFly)),
                {
...
/* harmony export */ });
/* harmony import */ var _flux_rendering_components__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./flux-rendering-components */ "./layout-editor/flux-rendering-components.ts");
...
function setupHtmlAutoUpdates(appStore, editor, htmlCodeEditor) {
    function update() {
        ...
        (0,_flux_rendering_components__WEBPACK_IMPORTED_MODULE_0__.replaceTemplateElements)(appStore.project.workflow.modules.map(m => m.moduleId), editor, appStore);
...
/* harmony import */ var _code_editors__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./code-editors */ "./layout-editor/code-editors.ts");
/* harmony import */ var _flux_blocks__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./flux-blocks */ "./layout-editor/flux-blocks.ts");
/* harmony import */ var _flux_rendering_components__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./flux-rendering-components */ "./layout-editor/flux-rendering-components.ts");
...
function plugCommands(editor, appStore) {
    ...
    editor.on('change', (element) => {
        let html = localStorage.getItem("gjs-html");
        if (appStore.project.runnerRendering.layout !== html && html != "") {
            debugSingleton.debugOn &&
                debugSingleton.logRenderTopic({
                    level: _builder_editor_builder_state_index__WEBPACK_IMPORTED_MODULE_1__.LogLevel.Info, message: "change => layout",
                    object: {
                        element,
        ...
        if (appStore.project.runnerRendering.style !== css && css != "") {
            debugSingleton.debugOn &&
                debugSingleton.logRenderTopic({
                    level: _builder_editor_builder_state_index__WEBPACK_IMPORTED_MODULE_1__.LogLevel.Info, message: "change => style",
                    object: {
                        element,
    ...
    editor.on('canvas:drop', (dataTransfer, component) => {
        ...
        if (mdle) {
            ...
            if (mdle instanceof _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.Component.Module) {
                childrenModulesId = mdle.getAllChildren(appStore.project.workflow).map(m => m.moduleId);
                editor.getStyle().add(mdle.getFullCSS(appStore.project.workflow, { asString: true }));
            }
            child.id = mdle.moduleId;
            (0,_flux_rendering_components__WEBPACK_IMPORTED_MODULE_5__.replaceTemplateElements)([child.id, ...childrenModulesId], editor, appStore);
            (0,_flux_blocks__WEBPACK_IMPORTED_MODULE_4__.setDynamicComponentsBlocks)(appStore, editor);
        }
    });
    editor.on('component:update:content', (data) => {
        //when inner html has changed, e.g. after text changed
        debugSingleton.debugOn &&
            debugSingleton.logRenderTopic({ level: _builder_editor_builder_state_index__WEBPACK_IMPORTED_MODULE_1__.LogLevel.Info, message: "component:update:content", object: { module: data } });
    });
    editor.on('component:update:style', (data) => {
        console.log("AAAAAAAAAAAAAAAAAAAAAA");
        debugSingleton.debugOn &&
            debugSingleton.logRenderTopic({ level: _builder_editor_builder_state_index__WEBPACK_IMPORTED_MODULE_1__.LogLevel.Info, message: "component:update:style", object: { module: data } });
    });
    editor.on('sorter:drag:end', ({ modelToDrop, srcEl }) => {
        ...
        if (!mdle)
            return;
        let moduleIds = (mdle instanceof _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.Component.Module)
            ? mdle.getAllChildren(appStore.project.workflow).map(m => m.moduleId)
            : [mdle.moduleId];
        (0,_flux_rendering_components__WEBPACK_IMPORTED_MODULE_5__.replaceTemplateElements)(moduleIds, editor, appStore);
    });
    ...
    editor.Commands.add('open-code', {
        ...
        run: function (editor, senderBtn) {
            ...
            if (!this.codePanel)
                this.codePanel = (0,_code_editors__WEBPACK_IMPORTED_MODULE_3__.buildCodePanel)(appStore, editor, panel);
...
/* harmony import */ var grapesjs__WEBPACK_IMPORTED_MODULE_12__ = __webpack_require__(/*! grapesjs */ "grapesjs");
/* harmony import */ var grapesjs__WEBPACK_IMPORTED_MODULE_12___default = /*#__PURE__*/__webpack_require__.n(grapesjs__WEBPACK_IMPORTED_MODULE_12__);
...
async function createLayoutEditor() {
    ...
    let editor = grapesjs__WEBPACK_IMPORTED_MODULE_12__.init({
        autorender: false,
        container: '#gjs',
        canvas: {
            styles: [],
    ...
    editor.render();
    return new Promise((successCb) => editor$.pipe((0,rxjs_operators__WEBPACK_IMPORTED_MODULE_11__.take)(1)).subscribe((edtr) => successCb(edtr)));
}
function initLayoutEditor(editor, { layout, style }, appStore) {
    let debugSingleton = _builder_editor_builder_state_index__WEBPACK_IMPORTED_MODULE_0__.AppDebugEnvironment.getInstance();
    debugSingleton.debugOn &&
        debugSingleton.logRenderTopic({
            level: _builder_editor_builder_state_index__WEBPACK_IMPORTED_MODULE_0__.LogLevel.Info,
...
/* harmony export */   "getFluxBlockContent": () => (/* binding */ getFluxBlockContent)
/* harmony export */ });
/* harmony import */ var _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! @youwol/flux-core */ "@youwol/flux-core");
/* harmony import */ var _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(_youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var _builder_editor_builder_state__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ../builder-editor/builder-state */ "./builder-editor/builder-state/index.ts");
/* harmony import */ var _utils__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./utils */ "./layout-editor/utils.ts");
...
function setDynamicComponentsBlocks(appStore, editor) {
    let debugSingleton = _builder_editor_builder_state__WEBPACK_IMPORTED_MODULE_1__.AppDebugEnvironment.getInstance();
    let all = (0,_utils__WEBPACK_IMPORTED_MODULE_2__.getAllComponentsRec)(editor);
    ...
    debugSingleton.debugOn &&
        debugSingleton.logRenderTopic({
            level: _builder_editor_builder_state__WEBPACK_IMPORTED_MODULE_1__.LogLevel.Info,
...
function toDynamicBlock(mdle, editor, workflow) {
    return {
        id: mdle.moduleId,
...
function getFluxBlockContent(mdle, editor, workflow) {
    let debugSingleton = _builder_editor_builder_state__WEBPACK_IMPORTED_MODULE_1__.AppDebugEnvironment.getInstance();
    debugSingleton.debugOn &&
        debugSingleton.logRenderTopic({
            level: _builder_editor_builder_state__WEBPACK_IMPORTED_MODULE_1__.LogLevel.Info,
...
/***/ "./layout-editor/flux-rendering-components.ts":
/*!****************************************************!*\
  !*** ./layout-editor/flux-rendering-components.ts ***!
  \****************************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {
...
/* harmony export */   "updateElementsInLayout": () => (/* binding */ updateElementsInLayout),
/* harmony export */   "autoAddElementInLayout": () => (/* binding */ autoAddElementInLayout),
/* harmony export */   "autoRemoveElementInLayout": () => (/* binding */ autoRemoveElementInLayout)
/* harmony export */ });
/* harmony import */ var _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! @youwol/flux-core */ "@youwol/flux-core");
/* harmony import */ var _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(_youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var _builder_editor_builder_state__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ../builder-editor/builder-state */ "./builder-editor/builder-state/index.ts");
/* harmony import */ var _flux_blocks__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./flux-blocks */ "./layout-editor/flux-blocks.ts");
/* harmony import */ var _utils__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./utils */ "./layout-editor/utils.ts");




function removeTemplateElements(modules, editor) {
    let allGjs = (0,_utils__WEBPACK_IMPORTED_MODULE_3__.getAllComponentsRec)(editor);
...
function replaceTemplateElements(moduleIds, editor, appStore) {
    ...
    (0,_youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.renderTemplate)(body, mdles.filter(mdle => !(mdle instanceof _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.Component.Module)), { applyWrapperAttributes: false });
    mdles
        .filter(mdle => mdle instanceof _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.Component.Module)
    ...
    let allGjsComponents = (0,_utils__WEBPACK_IMPORTED_MODULE_3__.getAllComponentsRec)(editor);
    mdles
...
function updateElementsInLayout(diff, editor, appStore) {
...
function autoAddElementInLayout(diff, editor, appStore) {
    ...
    let newIds = diff.createdElements
        .filter((mdle) => mdle.Factory.RenderView)
        .filter((mdle) => !removedIds.includes(mdle.moduleId))
        .filter((mdle) => appStore.getRootLayer().getModuleIds().includes(mdle.moduleId))
        .filter((mdle) => !(mdle instanceof _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.Component.Module))
        .map((mdle) => {
        let htmlContent = (0,_flux_blocks__WEBPACK_IMPORTED_MODULE_2__.getFluxBlockContent)(mdle, editor, appStore.project.workflow);
 ...
 */
function autoRemoveElementInLayout(diff, editor, appStore) {
    let removedIds = diff.removedElements.map(m => m.moduleId);
    let toRemove = diff.createdElements
        .filter((mdle) => mdle instanceof _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.Component.Module)
        .filter((mdle) => !removedIds.includes(mdle.moduleId))
        .map((mdle) => mdle.getDirectChildren(appStore.project.workflow))
        .flat();
...
/* harmony export */ });
let privateClasses = ["flux-element", "flux-component", "flux-fill-parent", "d-flex", "flex-column"];
...
function focusAction(mdle, appStore, workflowPlotter, toggledClass, duration = 5000) {
    ...
    let layer = appStore.project.workflow.modules
        .find((mdle) => mdle instanceof _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.GroupModules.Module && mdle.getModuleIds().includes(mdle.moduleId));
...
/* harmony import */ var _layout_editor_flux_rendering_components__WEBPACK_IMPORTED_MODULE_14__ = __webpack_require__(/*! ./layout-editor/flux-rendering-components */ "./layout-editor/flux-rendering-components.ts");
/* harmony import */ var _layout_editor_flux_blocks__WEBPACK_IMPORTED_MODULE_15__ = __webpack_require__(/*! ./layout-editor/flux-blocks */ "./layout-editor/flux-blocks.ts");
...
async function initializeRessources() {
    ...
    let environment = new _youwol_flux_core__WEBPACK_IMPORTED_MODULE_2__.Environment({ console /*: noConsole as Console*/,
...
function connectStreams(appStore, workflowPlotter, layoutEditor, appObservables) {
    ...
    appObservables.renderingLoaded$.subscribe(({ layout, style }) => {
        (0,_layout_editor_index__WEBPACK_IMPORTED_MODULE_8__.initLayoutEditor)(layoutEditor, { layout, style }, appStore);
    ...
    (0,rxjs__WEBPACK_IMPORTED_MODULE_0__.combineLatest)([layoutEditor$, appObservables.modulesUpdated$])
        ...
        (0,_layout_editor_flux_rendering_components__WEBPACK_IMPORTED_MODULE_14__.removeTemplateElements)(notReplaced, editor);
        if (loading)
            (0,_layout_editor_flux_rendering_components__WEBPACK_IMPORTED_MODULE_14__.replaceTemplateElements)(diff.createdElements.map((m) => m.moduleId), editor, appStore);
        if (!loading) {
            (0,_layout_editor_flux_rendering_components__WEBPACK_IMPORTED_MODULE_14__.autoAddElementInLayout)(diff, editor, appStore);
            (0,_layout_editor_flux_rendering_components__WEBPACK_IMPORTED_MODULE_14__.updateElementsInLayout)(diff, editor, appStore);
            (0,_layout_editor_flux_rendering_components__WEBPACK_IMPORTED_MODULE_14__.autoRemoveElementInLayout)(diff, editor, appStore);
        }
        (0,_layout_editor_flux_blocks__WEBPACK_IMPORTED_MODULE_15__.setDynamicComponentsBlocks)(appStore, editor);
    });
    (0,rxjs__WEBPACK_IMPORTED_MODULE_0__.combineLatest)([layoutEditor$, appObservables.activeLayerUpdated$])
        .subscribe(([editor, diff]) => {
        (0,_layout_editor_flux_blocks__WEBPACK_IMPORTED_MODULE_15__.setDynamicComponentsBlocks)(appStore, editor);
    });
    (0,rxjs__WEBPACK_IMPORTED_MODULE_0__.combineLatest)([layoutEditor$, appObservables.uiStateUpdated$]).pipe((0,rxjs_operators__WEBPACK_IMPORTED_MODULE_1__.filter)(([editor, state]) => state.mode === "combined" || state.mode === "render")).subscribe(([editor, state]) => (0,_layout_editor_flux_rendering_components__WEBPACK_IMPORTED_MODULE_14__.replaceTemplateElements)(appStore.project.workflow.modules.map(m => m.moduleId), editor, appStore));
    ...
    appObservables.ready$.subscribe(() => {
        (0,_builder_editor_index__WEBPACK_IMPORTED_MODULE_7__.createAttributesPanel)(appStore, appObservables);
    });
...
function createLayerPickerView(appStore, editor) {
    let subscriptions = [];
    function createContentRecursive(grpMdle) {
        ...
        const childrenLayers = grpMdle.getDirectChildren()
            .filter(mdle => mdle instanceof _youwol_flux_core__WEBPACK_IMPORTED_MODULE_0__.GroupModules.Module)
            .map((child) => createContentRecursive(child));
...
//# sourceMappingURL=on-load_ts.0a06c33e9cf2e5fbde23.js.map